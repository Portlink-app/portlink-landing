...
export default function TermsPage() {
  return (
    <SeatradeShell>
      ...
      <p style={p}>Complete the port call friction score at portlink.app/seatrade, agree to receive the Portlink newsletter, and confirm your email through the link we send you. The newsletter subscription is a condition of entry and is stated before the questions begin; you can unsubscribe at any time and your entry stays. A confirmed sign-up is one entry. Every person who signs up through your invitation link and confirms their own work email adds one entry for you, up to {REFERRAL_CAP} such entries; at most {SAME_DOMAIN_CAP} of them may come from your own email domain. Each person counts once, for whoever invited them first. One sign-up per person; duplicate or automated sign-ups, aliases and addresses that do not belong to the person entering are removed.</p>
      ...
      <p style={p}>Your name, work email, company and answers are used to run the draw, to send you your scorecard and the Portlink newsletter, which has no end date and which you can leave in one click, and, in anonymous aggregate, for the live benchmark. If you write an answer to either of the two open questions we show after your entry is in, that text is kept with your entry and read by Portlink; both are optional and leaving them blank changes nothing. Unsubscribing from emails does not withdraw your entry; reply to any email to withdraw or to have your data deleted. Data is stored on Netlify (EU/US) and emails are sent through Resend (EU region). Portlink&rsquo;s privacy contact: reply to pilot@portlink.app.</p>
